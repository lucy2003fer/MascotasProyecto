...
function mostrarMascotas(mascotas) {
  ...
  mascotas.forEach((mascota, index) => {
    ...
    petTemplate.querySelector(".pet-image").src = fotoUrl;
    petTemplate.querySelector(".pet-image").alt = mascota.name || 'Mascota sin nombre';
    petTemplate.querySelector(".pet-name").textContent = mascota.name || 'Mascota sin nombre';
    petTemplate.querySelector(".pet-raze").textContent = `Raza ID: ${mascota.race_id || "Sin raza especificada"}`;
    
    // Configurar botón de eliminar
    const deleteBtn = petTemplate.querySelector(".btn-delete");
    deleteBtn.addEventListener('click', () => eliminarMascota(mascota.id));
    
    listarpets.appendChild(petTemplate);
  });
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Configurar botón añadir (si es necesario)
  document.getElementById("btn-add").addEventListener('click', () => {
    // Aquí puedes añadir la lógica para agregar una nueva mascota
